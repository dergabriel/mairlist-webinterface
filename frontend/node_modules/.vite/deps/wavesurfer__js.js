...
var i = { decode: function(e2, i2) {
  ...
  if ("number" == typeof t2[0] && (t2 = [t2]), !t2[0] || 0 === t2[0].length)
  ...
  return { duration: e2, length: i2[0].length, sampleRate: i2[0].length / e2, numberOfChannels: i2.length, getChannelData: (t3) => {
    const e3 = i2[t3];
    if (!e3)
      throw new Error(`Channel ${t3} not found`);
    return e3;
...
function n(t2, e2) {
  ...
  for (const [t3, s2] of Object.entries(e2))
    if ("children" === t3 && s2)
      for (const [t4, e3] of Object.entries(s2))
        e3 instanceof Node ? i2.appendChild(e3) : "string" == typeof e3 ? i2.appendChild(document.createTextNode(e3)) : i2.appendChild(n(t4, e3));
    else
      "style" === t3 ? Object.assign(i2.style, s2) : "textContent" === t3 ? i2.textContent = s2 : i2.setAttribute(t3, s2.toString());
  return i2;
}
function s(t2, e2, i2) {
  const s2 = n(t2, e2 || {});
...
var a = { fetchBlob: function(e2, i2, n2) {
  return t(this, void 0, void 0, function* () {
    var s2;
    const r2 = yield fetch(e2, n2);
    if (r2.status >= 400)
      throw new Error(`Failed to fetch ${e2}: ${r2.status} (${r2.statusText})`);
    return function(e3, i3, n3) {
      t(this, void 0, void 0, function* () {
        ...
        try {
          ...
          if (t3 instanceof DOMException && "AbortError" === t3.name)
            return;
...
var u = class extends e {
  ...
  constructor(t2) {
    super(), this.isExternalMedia = false, this._ownBlobUrl = null, this.reactiveMediaEventCleanups = [], t2.media ? (this.media = t2.media, this.isExternalMedia = true) : this.media = document.createElement("audio"), this._isPlaying = l(false), this._currentTime = l(0), this._duration = l(0), this._volume = l(this.media.volume), this._muted = l(this.media.muted), this._playbackRate = l(this.media.playbackRate || 1), this._seeking = l(false), this.setupReactiveMediaEvents(), t2.mediaControls && (this.media.controls = true), t2.autoplay && (this.media.autoplay = true), null != t2.playbackRate && this.onMediaEvent("canplay", () => {
  ...
  setSrc(t2, e2) {
    ...
    if (n2 !== t2 && (this._ownBlobUrl = n2), i2 && this.media.removeAttribute("src"), n2 || t2)
      try {
        this.media.src = n2;
      } catch (e3) {
        this.media.src = t2;
  ...
  play() {
    return t(this, void 0, void 0, function* () {
      try {
        ...
        if (t2 instanceof DOMException && "AbortError" === t2.name)
          return;
        throw t2;
      }
    });
...
function d({ maxTop: t2, maxBottom: e2, halfHeight: i2, vScale: n2, barMinHeight: s2 = 0, barAlign: r2 }) {
...
function y(t2) {
  const e2 = l({ scrollLeft: t2.scrollLeft, scrollWidth: t2.scrollWidth, clientWidth: t2.clientWidth }), i2 = h(() => function(t3) {
...
var C = class extends e {
  ...
  parentFromOptionsContainer(t2) {
    let e2;
    if ("string" == typeof t2 ? e2 = document.querySelector(t2) : r(t2) && (e2 = t2), !e2)
  ...
  initDrag() {
    ...
    this.dragStream = function(t3, e2 = {}) {
      const { threshold: i2 = 3, mouseButton: n2 = 0, touchDelay: s2 = 100 } = e2, r2 = l(null), o2 = /* @__PURE__ */ new Map(), a2 = matchMedia("(pointer: coarse)").matches;
      let h2 = () => {
      };
      const c2 = (e3) => {
        if (e3.button !== n2)
          return;
        if (o2.has(e3.pointerId))
          return;
        if (o2.set(e3.pointerId, e3), o2.size > 1)
          return;
        ...
        const p2 = Date.now(), m2 = t3.getBoundingClientRect(), { left: g2, top: f2 } = m2, v2 = (t4) => {
          if (t4.pointerId !== l2)
            return;
          if (t4.defaultPrevented || o2.size > 1)
            return;
          if (a2 && Date.now() - p2 < s2)
            return;
  ...
  renderBarWaveform(t2, e2, i2, n2) {
    const { width: s2, height: r2 } = i2.canvas, { halfHeight: o2, barWidth: a2, barRadius: l2, barIndexScale: h2, barSpacing: c2, barMinHeight: u2 } = function({ width: t3, height: e3, length: i3, options: n3, pixelRatio: s3 }) {
      ...
      for (let t4 = 0; t4 <= u3; t4++) {
        ...
        if (u4 > g2) {
          const { topHeight: t5, totalHeight: e4 } = d({ maxTop: f2, maxBottom: v2, halfHeight: s3, vScale: r3, barMinHeight: l3, barAlign: a3 }), h4 = p({ barAlign: a3, halfHeight: s3, topHeight: t5, totalHeight: e4, canvasHeight: o3 });
    ...
    i2.beginPath();
    for (const t3 of m2)
      l2 && "roundRect" in i2 ? i2.roundRect(t3.x, t3.y, t3.width, t3.height, l2) : i2.rect(t3.x, t3.y, t3.width, t3.height);
  ...
  renderWaveform(t2, e2, i2) {
    if (i2.fillStyle = this.convertColorValues(e2.waveColor, i2), e2.renderFunction)
  ...
  reRender() {
    ...
    if (this.render(this.audioData), !this.isScrollable && this.scrollContainer.scrollLeft)
  ...
  scrollIntoView(t2, e2 = false) {
    ...
    if (this.isDragging) {
      ...
      if (e2 && this.options.autoCenter && t3 > 0) {
        ...
        if (void 0 === e3 || e3 <= 0)
          return void (this.scrollContainer.scrollLeft += t3);
        const n3 = s2 / e3;
...
var E = class extends e {
  constructor(t2) {
    super(), this.bufferNode = null, this.playStartTime = 0, this.playbackPosition = 0, this._muted = false, this._playbackRate = 1, this._duration = void 0, this.buffer = null, this.currentSrc = "", this.paused = true, this.crossOrigin = null, this.seeking = false, this.autoplay = false, this.addEventListener = this.on, this.removeEventListener = this.un, this._destroyed = false, function() {
      ...
      if (null == t3 ? void 0 : t3.audioSession)
        try {
...
var w = class _w extends u {
  ...
  constructor(t2) {
    ...
    const { state: i2, actions: n2 } = function(t3) {
      ...
      const a2 = null !== (e3 = null == t3 ? void 0 : t3.currentTime) && void 0 !== e3 ? e3 : l(0), c2 = null !== (i3 = null == t3 ? void 0 : t3.duration) && void 0 !== i3 ? i3 : l(0), u2 = null !== (n3 = null == t3 ? void 0 : t3.isPlaying) && void 0 !== n3 ? n3 : l(false), d2 = null !== (s3 = null == t3 ? void 0 : t3.isSeeking) && void 0 !== s3 ? s3 : l(false), p2 = null !== (r3 = null == t3 ? void 0 : t3.volume) && void 0 !== r3 ? r3 : l(1), m2 = null !== (o2 = null == t3 ? void 0 : t3.playbackRate) && void 0 !== o2 ? o2 : l(1), g2 = l(null), f2 = l(null), v2 = l(""), b2 = l(0), y2 = l(0), C2 = h(() => !u2.value, [u2]), S2 = h(() => null !== g2.value, [g2]), E2 = h(() => S2.value && c2.value > 0, [S2, c2]), P2 = h(() => a2.value, [a2]), w2 = h(() => c2.value > 0 ? a2.value / c2.value : 0, [a2, c2]);
  ...
  loadAudio(e2, n2, s2, r2) {
    return t(this, void 0, void 0, function* () {
      ...
      if (this._isDestroyed = false, this.emit("load", e2), !this.options.media && this.isPlaying() && this.pause(), this.decodedData = null, this.stopAtPosition = null, null === (t2 = this.abortController) || void 0 === t2 || t2.abort(), this.abortController = null, !n2 && !s2) {
        ...
        if (n2 = yield a.fetchBlob(e2, i2, t3), this._isDestroyed || o2 !== this._loadVersion)
          return;
      ...
      if (!this._isDestroyed && o2 === this._loadVersion) {
        ...
        if (s2)
          this.decodedData = i.createBuffer(s2, l2 || 0);
        else if (n2) {
